...
def rand_nsteps(explprob0, memstore0, memshare0, mem0, pulls0, payprobs0, numagents0, nsteps0, rng0):
    ...
    explarray0 = rng0.choice(2, (nsteps0, numagents0), p=[1-explprob0, explprob0])
    ...
    return payouts0, randunif00, perm00, store00, explarray0
...
@numba.jit
def agentst0FAST(memshare3, memevid3, nag3, con3):
    tolpmfs3 = np.zeros((nag3, nag3, memshare3+1))
    for idx30 in numba.prange(0, nag3):
        for idx31 in range(0, nag3):
            if idx30 == idx31:
...
@numba.jit
def paramsupdate(vdisc4, memplay4, memevidplay4, perms4, memsuc4, memarms4, store4, memstore4, memshare4, pulls4, nag4, randunif4, tolpmfs4, memevid4, memevidarms4, params4):
        ...
        for idx41 in range(0, nag4):
            for idx42 in range(0, memshare4+1):
                if idx42 == 0:
                    ...
                    if randunif4[idx40][idx41][idx42] < tolpmfs4[idx40][idx41][idx42]:
                        memevididx4 = floor(memevidarms4[idx42][idx41])
                        params4[idx40][memevididx4][0] += (memevid4[idx42][idx41])/vdisc4
                        params4[idx40][memevididx4][1] += (pulls4- memevid4[idx42][idx41])/vdisc4
                        storeidx = idx42+idx42*idx41
                        if storeidx in store4[idx40]:
                            replaceidx = floor(perms4[idx40][storecount])
                            memsuc4[idx40][replaceidx] = memevid4[idx42][idx41]
...
@numba.jit
def params_nsteps(explarray9, vdisc9, memplay9, idxf9, narms9, store9, perms9, memsuc9, memarms9, memshare9, memstore9, pulls9, nag9, con9, randunif90, pay9, params9, tol9, nsteps9):
    for idx90 in range(0, nsteps9):
        expect9 = agentexp(params9)
        evid9, evidarms9, evidplay9 = evid(pay9[idx90], expect9, idxf9, idx90, explarray9[idx90])
...
#         tolpmfs9 = agentstolpmfsCOARSE(agpmfCORtols9, memshare9, pulls9, nag9, params9, memevid9, memevidarms9, con9, tol9)
        tolpmfs9 = agentst0FAST(memshare9, memevid9, nag9, con9)
        params9, memsuc9, memarms9, memplay9 = paramsupdate(vdisc9, memplay9, memevidplay9, perms9[idx90], memsuc9, memarms9, store9[idx90], memstore9, memshare9, pulls9, nag9, randunif90[idx90], tolpmfs9, memevid9, memevidarms9, params9)
...
def confirm_bias_full(explprobf, vdiscf, mem_snapf, mem_depthf, erpf, memf, memsharef, memstoref, networktypef, maxalphabetaf, pullsf, nagf, narmsf, tolf, payprobsf, nstepsf, tstepsf, rngs, n0):
    ...
    for idxf in range(0, tstepsf, nstepsf):
        if idxf > (tstepsf - 3*nstepsf):
            explprobf = 0
        payf, randunif0f, permsf, storef, explarrayf = rand_nsteps(explprobf, memstoref, memsharef, memf, pullsf, payprobsf, nagf, nstepsf, rngf)
        paramsf, evidarmsf, memsucf, memarmsf, memplayf = params_nsteps(explarrayf, vdiscf, memplayf, idxf, narmsf, storef, permsf, memsucf, memarmsf, memsharef, memstoref, pullsf, nagf, connections, randunif0f, payf, paramsf, tolf, nstepsf)
        mem_ds = persistence(idxf, numba.typed.List(mem_depthf), numba.typed.List(mem_snapf), memplayf, mem_ds, md_len, ms_len, nagf, memf)
